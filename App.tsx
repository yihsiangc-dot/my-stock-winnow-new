...
const App: React.FC = () => {
  ...
  }, [activeSectorId, refreshMarketData]);
  ...
  const handleAnalysis = async () => {
    ...
    try {
      ...
      alert("AI 分析連線失敗。請確認 Vercel 環境變數 API_KEY 是否已設定。");
    } finally {
    ...
    <div className="min-h-screen bg-[#020202] text-white flex flex-col overflow-hidden">
      ...
      <header className="h-16 flex-shrink-0 bg-[#080808]/90 backdrop-blur-xl border-b border-white/5 flex items-center justify-between px-6 z-40">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => setShowSetupGuide(true)} className="p-2 text-slate-400 hover:text-white" title="修復佈署套件">
          ...
          <div className="max-w-5xl mx-auto space-y-12 pb-32">
            ...
            {apiStatus === 'error' && (
              <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-2xl flex items-center gap-4">
                ...
                <p className="text-xs text-red-400">行情同步異常：Vercel 需要配置 Environment Variables 才能獲取實時數據。</p>
              </div>
            )}
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
              ...
              <button 
                ...
                className="w-full lg:w-auto px-8 py-4 bg-red-600 rounded-2xl font-black uppercase text-xs tracking-widest hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:scale-100 shadow-[0_0_20px_rgba(239,68,68,0.3)]"
              >
                {isAnalyzing ? "AI Scanning Markets..." : "Request AI Deep Analysis"}
              </button>
            </div>
